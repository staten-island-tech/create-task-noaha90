...
async function displayMain(id){
  ...
  const data = await response.json(); 
  document.getElementById("info").insertAdjacentHTML("afterbegin",`<div class="infobox"><h2>${data.meals[0].strMeal}</h2><div class="foodInfo"><p>Type: ${data.meals[0].strCategory}</p><p>Region: ${data.meals[0].strArea}</p></div><img id="${id}"src="${data.meals[0].strMealThumb}"><p>${data.meals[0].strInstructions}</p></div>`)
}
